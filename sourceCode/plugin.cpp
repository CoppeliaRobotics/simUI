...
public:
    ...
    void create(create_in *in, create_out *out)
    {
        ...
        std::string scriptType = sim::getStringProperty(in->_.scriptID, "detachedScript.type");
    ...
    void setProperties(setProperties_in *in, setProperties_out *out)
    {
        ...
        SIM::getInstance()->setProperties(properties, in->pnames, in->ptypes, in->pvalues, in->pflags, in->pdisplayk, in->pdisplayv, in->icons, in->descriptions, in->suppressEvents);
#endif
    }
...
#if WIDGET_PROPERTIES
        Properties *properties = getWidget<Properties>(in->handle, in->id, "properties");
        SIM::getInstance()->setPropertiesRows(properties, in->rows, in->pnames, in->ptypes, in->pvalues, in->pflags, in->pdisplayk, in->pdisplayv, in->icons, in->descriptions, in->suppressEvents);
#endif
    }
